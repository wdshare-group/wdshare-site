...
function GetStyleOverridesForState(id, parentId, state) {
    ...
    if (isHyperlink) {
        var hyperlinkDefaultProps = { };
        if (state == 'mouseOver') {
            $.extend(hyperlinkDefaultProps, $axure.pageData.stylesheet.defaultStyles.hyperlinkMouseOver);
        } else if (state == 'mouseDown') {
            $.extend(hyperlinkDefaultProps, $axure.pageData.stylesheet.defaultStyles.hyperlinkMouseOver);
            $.extend(hyperlinkDefaultProps, $axure.pageData.stylesheet.defaultStyles.hyperlinkMouseDown);
        }
        if (!$.isEmptyObject(hyperlinkDefaultProps)) returnVal = $.extend(returnVal || { }, hyperlinkDefaultProps);
    }

    if (parentId) {
        var parent = $axure.pageData.scriptIdToObject[parentId];
        if (parent) {
            var parentStateStyle = parent && parent.getStateStyleOverrides(state);
            if (parentStateStyle) {
                returnVal = $.extend(returnVal || { }, parentStateStyle);
            }
        }
    }

    var diagramObject = $axure.pageData.scriptIdToObject[id];
    var diagramObjectStateStyle = diagramObject && diagramObject.getStateStyleOverrides(state);

    if (diagramObjectStateStyle) returnVal = $.extend(returnVal || { }, diagramObjectStateStyle);
